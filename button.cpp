...
button::button(int _j, int _i, field* _parent) : i(_i), j(_j), parent(_parent), is_pushed(false) {
    BN_ID = get_bn_id(j, i);
    symbol = 2;
    InitWindow();
}
...
field::field(int field_size) : _size(field_size) {
    is_first = true;
...
bool (*go[4][2])(size_t& i, size_t& j, size_t size) = {Nord, South, NordWest, SouthEast,
                                                       NordEast, SouthWest, East, West};

bool field::Go(int chek_symbol, size_t i, size_t j,
    ...
    for (int k = 0; k < 2; ++k) {
        ...
        do {
            ++count;
            if (!change_i_j[k](temp_i, temp_j, _size))
                break;
